...
public class AddP implements Initializable {
    ...
    void add_produit(ActionEvent event) {
        ...
        if (nomprod.isEmpty()) {
            ...
            if (file != null) {
                String destPath = "C:\\Users\\khalil\\Desktop\\JavaFX\\src\\main\\resources\\com\\example\\app\\images";
    ...
    void afficher_prod(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/app/AfficherProd.fxml"));
            Parent root = javafx.fxml.FXMLLoader.load(getClass().getResource("/com/example/app/AfficherProd.fxml"));
            Scene scene = new Scene(root);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(scene);
            stage.show();
